import type {WasmModule} from './faceDetectWasmModule'
...
export function getFaceDetectTransformer (faceDetectModule: WasmModule): (
  ...
  return (videoFrame, controller) => {
    try{
    const opts = {rect: {x:0, y:0, width: videoFrame.codedWidth, height: videoFrame.codedHeight}}
    ...
    // )
    videoFrame.copyTo(faceDetectModule.HEAPU8.subarray(inputImageOffset, inputImageOffset + size), opts)
    try{
      faceDetectModule.detectFace(videoFrame.codedWidth, videoFrame.codedHeight)
    }catch (ex) {
      if(typeof(ex) === 'number') {
    ...
    const output = faceDetectModule.HEAPU8.subarray(outputImageOffset, outputImageOffset + size)
